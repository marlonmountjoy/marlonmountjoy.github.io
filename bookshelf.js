...
function add() { // This is by MarMar using the authors original open license book div code as bookDetails feel free to use
  ...
  let required = document.getElementById("btnAdd").addEventListener("mousedown", validateForm)
  function validateForm() {
    let url = document.getElementById('txtCover').value;
    let title = document.getElementById('txtTitle').value;
    let author = document.getElementById('txtAuthor').value;
    if (url == "") {
      alert("Cover must be a url");
      return false;
      window.stop();
    } else if (title == "") {
